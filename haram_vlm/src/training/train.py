...
def train():
    ...
    # Propagate HARAM flag to the data pipeline so it emits metadata tensors.
    data_args.haram_metadata = training_args.haram_enable
    ...
    if training_args.haram_enable:
        from training.haram_model import HaramForTraining, HaramTrainer
        hidden_size = getattr(model.config, "hidden_size", None)
        if hidden_size is None:  # PEFT-wrapped configs may nest it
            hidden_size = model.config.text_config.hidden_size
        rank0_print(f"Enabling HARAM multi-task training (hidden_size={hidden_size})...")
        haram_model = HaramForTraining(
            base_model=model,
            hidden_size=hidden_size,
            w_halluc=training_args.haram_halluc_weight,
            w_router=training_args.haram_router_weight,
            w_token=training_args.haram_token_weight,
        )
        # Keep the lightweight HARAM heads in fp32 for numerically stable aux losses.
        haram_model.head.to(device=training_args.device, dtype=torch.float32)
        trainer = HaramTrainer(
            model=haram_model,
            processor=processor,
            args=training_args,
            **data_module
    ...
    if training_args.haram_enable:
        # HaramTrainer._save persists the LoRA adapter + the HARAM head.

...
def encode_video(video_path, max_num_frames=10):
    if not HAS_DECORD:
        # Return empty list if decord is not available
        return []
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        record_metadata = sources.get("metadata", {}) if isinstance(sources, dict) else {}
        ...
        # HARAM auxiliary supervision: emit per-sample metadata tensors.
        if getattr(self.data_args, "haram_metadata", False):
            from training.haram_model import query_type_to_id
            data_dict["resolution"] = torch.tensor(
                int(record_metadata.get("resolution", 448)), dtype=torch.long
            )
            data_dict["halluc_risk"] = torch.tensor(
                float(record_metadata.get("expected_hallucination_risk", 0.1)), dtype=torch.float
            )
            data_dict["query_type_id"] = torch.tensor(
                query_type_to_id(record_metadata.get("query_type", "")), dtype=torch.long
            )
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, examples):
        ...
        # Collate HARAM metadata tensors when present.
        for key in ("resolution", "halluc_risk", "query_type_id"):
            if key in examples[0]:
                batch_dict[key] = torch.stack([example[key] for example in examples])
